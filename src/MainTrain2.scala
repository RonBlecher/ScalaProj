...
object MainTrain2 {
    def testTimeSeries() : Unit = {
        val ts = new TimeSeries("csv/train5.csv")
    ...
    def main(args: Array[String]): Unit = {
        testTimeSeries()
        val ts = new TimeSeries("csv/train6.csv")
        testEAD(ts,1)
        testEAD(ts,3)
        testEAD(ts,5)
        println("done")
